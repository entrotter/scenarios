...
class FrozenCaseTests(unittest.TestCase):
    def test_frozen_inputs_and_split_are_unchanged(self):
        ...
        self.assertEqual(hashlib.sha256(data).hexdigest(), 'f147489fcde8de04be6a9de459fe011018488bd75e66a84de55f6a8a35ed030e')
        manifest = json.loads(data)
        self.assertEqual(len(manifest['cases']), 5)
        self.assertEqual([x['source']['block_number'] for x in manifest['cases'] if x['split'] == 'holdout'], [20000000, 21000000])
        self.assertEqual([x['source']['block_number'] for x in manifest['cases'] if x['split'] == 'development'], [19000000])
        for case in manifest['cases']:
            path = ROOT / case['path']
            self.assertEqual(hashlib.sha256(path.read_bytes()).hexdigest(), case['sha256'])
            scenario = validate(json.loads(path.read_text()))
            self.assertEqual(scenario['source']['block_hash'], case['source']['block_hash'])
